...
export async function language(root: string): Promise<void> {
  // eslint - disable-next-line unicorn/text-encoding-identifier-case

  await Promise.all(
    files.map(async ([input, encoding]) => {
      const iconv = new Iconv(encoding, 'utf8');
      const docs = await readDocumentation(root, `moby-${input}`);

      return fs.readFile(path.join(root, 'moby', 'mlang', `${input}.txt`)).then(async (buffer) => {
        const words: Set<string> = new Set();

        for (let line of splitLines(iconv.convert(buffer).toString())) {
          if (line) {
            switch (input) {
              case 'french': {
