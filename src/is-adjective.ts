...
/**
 * Determines if a word is an adjective (code 'A') according to the Moby Parts of Speech dataset
 ...
 * @returns True if the word is an adjective, otherwise false.
 *
 * @example
 * isAdjective('quick'); // true
 * isAdjective('run'); // false
 *
 * @group Parts of Speech
 * @category Classification
 */
export function isAdjective(word: string): boolean {
  return partOfSpeech(word)?.includes('A') ?? false;
}
