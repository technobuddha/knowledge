...
/**
 * Determines if a word is a noun phrase (code 'h') according to the Moby Parts of Speech dataset
 ...
 * @returns True if the word is a noun phrase, otherwise false.
 *
 * @example
 * isNounPhrase('by and large'); // true
 * isNounPhrase('run'); // false
 *
 * @group Parts of Speech
 * @category Classification
 */
export function isNounPhrase(word: string): boolean {
  return partOfSpeech(word)?.includes('h') ?? false;
}
