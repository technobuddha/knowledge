...
/**
 * Determines if a word is a definite article (code 'D') according to the Moby Parts of Speech dataset
 ...
 * @returns True if the word is a definite article, otherwise false.
 *
 * @example
 * isDefiniteArticle('the'); // true
 * isDefiniteArticle('a'); // false
 *
 * @group Parts of Speech
 * @category Classification
 */
export function isDefiniteArticle(word: string): boolean {
  return partOfSpeech(word)?.includes('D') ?? false;
}
