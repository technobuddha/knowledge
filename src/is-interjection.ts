...
/**
 * Determines if a word is an interjection (code '!') according to the Moby Parts of Speech dataset
 ...
 * @returns True if the word is an interjection, otherwise false.
 *
 * @example
 * isInterjection('wow'); // true
 * isInterjection('run'); // false
 *
 * @group Parts of Speech
 * @category Classification
 */
export function isInterjection(word: string): boolean {
  return partOfSpeech(word)?.includes('!') ?? false;
}
