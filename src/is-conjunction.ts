...
/**
 * Determines if a word is a conjunction (code 'C') according to the Moby Parts of Speech dataset
 ...
 * @returns True if the word is a conjunction, otherwise false.
 *
 * @example
 * isConjunction('and'); // true
 * isConjunction('run'); // false
 *
 * @group Parts of Speech
 * @category Classification
 */
export function isConjunction(word: string): boolean {
  return partOfSpeech(word)?.includes('C') ?? false;
}
